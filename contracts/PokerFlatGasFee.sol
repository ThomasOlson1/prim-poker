 ...
 * @dev Texas Hold'em Poker with flat gas fee model
 * Flat fee extracted from blinds, winner gets 100% of pot
 */
contract PokerFlatGasFee {

    // FLAT FEE - Same for ALL stakes!
    uint256 public constant GAS_FEE = 0.00035 ether; // ~$0.70
    ...
    function createTable(
        ...
        // Require minimum blinds to cover gas fee
        require(totalBlinds > GAS_FEE, "Blinds too low to cover gas");
    ...
    function startNewHand(uint256 tableId) external onlyGameServer {
        ...
        // Extract FLAT gas fee
        uint256 gasFee = GAS_FEE;
        ...
        returns (bool viable, string memory reason)
    {
        ...
        if (totalBlinds <= GAS_FEE) {
            return (false, "Blinds too small to cover gas fee");
        }

        uint256 toPot = totalBlinds - GAS_FEE;
        uint256 effectiveRake = (GAS_FEE * 100) / toPot;
